...
%% Reading files
dir_date_str = 'extreme case'; % Directory where the experimental data is
...
for k = 1:length(filenames)
    ...
    for freq_i = 1:length(freq_range)
        ...
        % end
            ...
            % figure(1);
            % plot(f_fft,mag2db(abs(out_fft./in_fft2)))
            % hold on
            % grid on
    ...
    % figure(1)
    % subplot(2,1,1)
    % semilogx(freq_range,mag2db(abs(H_Y2)),'LineWidth',2)
    % hold on
    ...
    %End-effector acccleration to q desired
%     figure(k) 
%     input = freq_range*sqrt(-1);
%     i = 2*sqrt(-1);
%     %f1 = @(x,freq_range)abs((input.^2)./(input.^2+x(1)*input+272.25).*(input+x(4))./(input.^2+x(2)*input+x(3)))/abs(i^2/(i^2+x(1)*i+272.25)*(i+x(4))/(i^2+x(2)*i+x(3)))
...
% if k == 1
% 
% mass = ur5e_inertia_matrix_5kg(0,0*pi/180,-163*pi/180,-120*pi/180,0,0)
% M11 = mass(1,1);
% M12 = mass(1,2);
% M22 = mass(2,2);
% M23 = mass(2,3);
...
% 
% figure(11)
% index = find(freq_range<29);
%     f = freq_range(index);
%     H = H_Y1_full(index)
% 
% i = f*sqrt(-1);
% 
% f1 = @(x,f)abs( x(3)./(i.^2+6*i+x(2)));
% x0 = [1,500,1];
% 
% x = lsqcurvefit(f1,x0,f,abs(H));
% 
% i = f*sqrt(-1);
% semilogx(freq_range,mag2db(abs(H_Y1_full)/H_Y1_full(1)),'LineWidth',2)
% hold on
% 
% %semilogx(f,mag2db(abs((x(3)./(i.^2+x(1)*i+x(2))))),'LineWidth',2)
% %semilogx(f,mag2db(abs(     (i(1).^2+6*i(1)+x(2)) ./(i.^2+6*i+x(2))       )    ),'LineWidth',2)
% semilogx(f,mag2db(abs(     (i(1).^2+6*i(1)+515) ./(i.^2+6*i+515)       )    ),'LineWidth',2)
% 
% 
% % 
% hold on
...
%     ylabel('Mag [dB]')
%     grid on
%     title('End effector Displacement to Base Desired FRF in Base Excitation')
%     legend('Experiment','Plot Fitting','-3db')
% 
% 
%     %filename = 'FRF_base_'+string(filenames(1))+'.mat';
%     %save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
%  end
...
index = find(freq_range<30);
    f = freq_range_rad(index);
    H = H_Y1_full(index);

i = f*sqrt(-1);
f1 = @(x,f)abs( x(3)./(i.^2+x(1)*i+x(2)));
x0 = [1,900,1];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
H_B_fit     = zeros(length(H_Y1_full),1);
freq_range_fit = horzcat(0.001, freq_range);
index_pole1      = find(freq_range_fit <= 30);

num_order_base=1;
den_order_base=3;

%freq_range_rad            = 2*pi*freq_range; % Frequncy vector to simulate the FRF of the model
...
H_base_fit    = horzcat(1, H_Y1_full/H_Y1_full(1)); % Accounting for the extra 0.001 we added in the frequency vector
...
H_B_fit  = freqs(b_fit,a_fit,w_fit); % Simulating the FRF of the model 
sys           = tf(b_fit,a_fit);
...
semilogx(freq_range,mag2db(abs(H_Y1_full)/H_Y1_full(1)),'LineWidth',2)
hold on
semilogx(freq_range,mag2db(abs(     (i(1).^2+x(1)*i(1)+x(2)) ./(i.^2+x(1)*i+x(2))       )    ),'LineWidth',2)

hold on
semilogx(freq_range_fit,mag2db(abs(H_B_fit/H_B_fit(1))),'LineWidth',2);
    xlim([1,100])
    hold on
    ylim([-20,20])
    yline(-3,'r-.','-3 dB','LineWidth',2);hold off
    ...
    grid on
    title('End effector Displacement to Base Desired FRF in Base Excitation')
    legend('Experiment','Plot Fitting using lsq','Plot Fitting using invfreq','-3db')

subplot(2,1,2)
semilogx(freq_range,unwrap(angle(H_Y1_full))/pi*180,'LineWidth',2)
hold on
semilogx(freq_range,unwrap(angle(1./(i.^2+x(1)*i+x(2))))/pi*180,'LineWidth',2)
hold on
...
% M = 7.6892;
% 
% mass = ur5e_inertia_matrix_5kg(0,0,0,0,0,0)
% M11 = mass(1,1);
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
% hold on
% % semilogx(freq_range,mag2db(abs(f1(x,freq_range))),'LineWidth',2)
% semilogx(freq_range,mag2db(abs(1./(i.^2+c*i+K/M))/abs(1./(i(1).^2+c*i(1)+K/M))),'LineWidth',2)
% %semilogx(f,mag2db(abs(H_Y1)-abs((x(3)./(i.^2+x(1)*i+x(2))))),'LineWidth',2)
% % 
% % 
% hold on
%     ylim([-20,20])
...
%     legend('Experiment','Plot Fitting','-3db')
% 
%     filename = 'FRF_base_'+string(filenames(2))+'.mat';
%     save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
% end
% 
% 
...
% M = 7.6931;
% 
% mass = ur5e_inertia_matrix_5kg(0,0,-pi/9,0,0,0)
% M11 = mass(1,1);
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
% M = 6.9480;
% 
% mass = ur5e_inertia_matrix_5kg(0,0,-2*pi/9,0,0,0)
% M11 = mass(1,1);
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
%     title('End effector Displacement to Base Desired FRF in Base Excitation')
%     legend('Experiment','Plot Fitting','-3db')
%     filename = 'FRF_base_'+string(filenames(3))+'.mat';
%     save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
% end
% 
% if k == 4
...
% M = 5.6655;
% 
% mass = ur5e_inertia_matrix_5kg(0,0,-3*pi/9,0,0,0)
% M11 = mass(1,1);
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
%     title('End effector Displacement to Base Desired FRF in Base Excitation')
%     legend('Experiment','Plot Fitting','-3db')
%     filename = 'FRF_base_'+string(filenames(4))+'.mat';
%     save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
% end
% 
% if k == 5
% figure(5)
% 
% mass = ur5e_inertia_matrix_5kg(0,0,-4*pi/9,0,0,0)
% M11 = mass(1,1);
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
%     title('End effector Displacement to Base Desired FRF in Base Excitation')
%     legend('Experiment','Plot Fitting','-3db')
%     filename = 'FRF_base_'+string(filenames(5))+'.mat';
%     save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
% end
% 
% 
...
% M = 2.8660;
% 
% mass = ur5e_inertia_matrix_5kg(0,0,-5*pi/9,0,0,0)
% M11 = mass(1,1);
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
%     title('End effector Displacement to Base Desired FRF in Base Excitation')
%     legend('Experiment','Plot Fitting','-3db')
%     filename = 'FRF_base_'+string(filenames(6))+'.mat';
%     save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
% end
% 
% if k == 7
...
% M = 1.9232;
% 
% mass = ur5e_inertia_matrix_5kg(0,0,-6*pi/9,0,0,0)
% M11 = mass(1,1);
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
%     title('End effector Displacement to Base Desired FRF in Base Excitation')
%     legend('Experiment','Plot Fitting','-3db')
%     filename = 'FRF_base_'+string(filenames(7))+'.mat';
%     save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
% end
% 
% 
...
% M = 1.4114;
% 
% mass = ur5e_inertia_matrix_5kg(0,0,-7*pi/9,0,0,0)
% M11 = mass(1,1)
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
%     title('End effector Displacement to Base Desired FRF in Base Excitation')
%     legend('Experiment','Plot Fitting','-3db')
%     filename = 'FRF_base_'+string(filenames(8))+'.mat';
%     save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
% end
% 
% if k == 9
% figure(9)
% mass = ur5e_inertia_matrix_5kg(0,0,-8*pi/9,0,0,0)
% M11 = mass(1,1)
% M12 = mass(1,2);
% M22 = mass(2,2);
% 
...
%     title('End effector Displacement to Base Desired FRF in Base Excitation')
%     legend('Experiment','Plot Fitting','-3db')
%     filename = 'FRF_base_'+string(filenames(9))+'.mat';
%     save(filename,"H_Y1",'H_Y1_full',"H_Y1_joint","R_m","V_rad","freq_range")
% end
...
% 
% %     save data
%save('experiment result\1',"H_Y1_full")
