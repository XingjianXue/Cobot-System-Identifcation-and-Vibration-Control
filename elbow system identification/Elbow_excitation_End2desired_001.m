...
%% Reading files
dir_date_str = '8.8-1'; % Directory where the experimental data is
...
mass = ur5e_inertia_matrix_5kg(0,-(k-1)*pi/9,2*(k-1)*pi/9,0,0,0)
M11 = mass(1,1);
M12 = mass(1,2);
M22 = mass(2,2);
M23 = mass(2,3);
...
%(K1*K2 - M12^2*s^4 + K2*M11*s^2 + K1*M22*s^2 + M11*M22*s^4 + C1*M11*M22*s^3 + C2*M11*M22*s^3 + C1*K2*M11*s + C2*K1*M22*s + C1*C2*M11*M22*s^2)


q = [M11*M22-M12^2  C1*M11*M22+C2*M11*M22  C1*C2*M11*M22+K2*M11+K1*M22 C1*K2*M11+C2*K1*M22  K1*K2];
...
semilogx(freq_range,mag2db(abs(H_Y3_full)/abs(H_Y3_full(1))),'LineWidth',2)
hold on
...
hold on
    ...
    ylabel('Mag [dB]')
    grid on
    title('End effector Displacement to Elbow Desired FRF')
