...
%% Reading files
dir_date_str = '8.2-3'; % Directory where the experimental data is
...
mass = ur5e_inertia_matrix_5kg(0,-3*pi/9,-(2*k-8)*pi/9,0,0,0)
M11 = mass(1,1);
M12 = mass(1,2);
M22 = mass(2,2);
M23 = mass(2,3);
...
%q = [M11*M33+M12*M13-M11*M23-M13^2  C1*M11*M33+C3*M11*M33-C1*M11*M23  C1*C3*M11*M33-K1*M23+K3*M11+K1*M33  C1*K3*M11+C3*K1*M33    K1*K3];
