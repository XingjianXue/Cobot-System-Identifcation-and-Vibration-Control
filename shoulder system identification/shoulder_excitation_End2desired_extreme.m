...
%% Reading files
dir_date_str = 'extreme_shoulder'; % Directory where the experimental data is
...
%     figure(k) 
%     index = find(freq_range<120)
%     freq_range = freq_range(index)
%     input = freq_range*sqrt(-1);
%     i = 2*sqrt(-1);
%     %f1 = @(x,freq_range)abs((input.^2)./(input.^2+x(1)*input+196).*(input+x(4))./(input.^2+x(2)*input+x(3)))/abs(i^2/(i^2+x(1)*i+196)*(i+x(4))/(i^2+x(2)*i+x(3)))
%     f1 = @(x,freq_range)abs((input.^2)./(input.^2+x(1)*input+x(2)));
%     x0 = [1,1,1,1];
%     x = lsqcurvefit(f1,x0,freq_range,abs(H_Y2(index)))
% 
%     subplot(2,1,1)
%     semilogx(freq_range,mag2db(abs(H_Y2(index))),'LineWidth',2)
...
%     ylabel('Mag [dB]')
%     grid on
%     title('End effector acceleration To Shoulder Joint FRF')
%     legend('Experiment FRF','Fitting FRF','Location','southwest')
% 
% 
% %     save data
...
%     i = 2*sqrt(-1);
%     %f1 = @(x,freq_range)abs((input.^2)./(input.^2+x(1)*input+196).*(input+x(4))./(input.^2+x(2)*input+x(3)))/abs(i^2/(i^2+x(1)*i+196)*(i+x(4))/(i^2+x(2)*i+x(3)))
%     f1 = @(x,freq_range)abs((input.^2)./(input.^2+x(1)*input+x(2)))/abs(i^2/(i^2+x(1)*i+x(2)));
%     x0 = [1,1,1,1];
%     x = lsqcurvefit(f1,x0,freq_range,abs(H_Y2(index)))
% 
%     subplot(2,1,2)

%semilogx(freq_range,mag2db(abs(H_Y2_full/H_Y2_full(1))),'LineWidth',2)
%     %semilogx(freq_range,abs(H_Y2),'LineWidth',2)
...
% 
% hold on
% fit = (M33*s.^2 + C3*M33*s + K3 -M23*s.^2)./(K2*K3 - M23^2*s.^4 + K3*M22*s.^2 + K2*M33*s.^2 + M22*M33*s.^4 + C2*M22*M33*s.^3 + C3*M22*M33*s.^3 + C2*K3*M22*s + C3*K2*M33*s + C2*C3*M22*M33*s.^2) 
% semilogx(freq_range,mag2db(  abs(fit/fit(1))),'LineWidth',2)
% hold on
% end
% 
% 
...
%     ylim([-20,10])
%     grid on
%     title('End effector Displacement To Shoulder Joint FRF in Shoulder Excitation')
...
% 
% 
% %K1*K3 - M13^2*s^4 + K3*M11*s^2 + K1*M33*s^2 + M11*M33*s^4 + C1*M11*M33*s^3 + C3*M11*M33*s^3 + C1*K3*M11*s + C3*K1*M33*s + C1*C3*M11*M33*s^2
% 
% %-(K1*M23*s^2 - M12*M13*s^4 + M11*M23*s^4 + C1*M11*M23*s^3)
% 
% q = [M11*M33+1/2*M12*M13-1/2*M11*M23-M13^2  C1*M11*M33+C3*M11*M33-1/2*C1*M11*M23  C1*C3*M11*M33-1/2*K1*M23+K3*M11+K1*M33  C1*K3*M11+C3*K1*M33    K1*K3];
% 
% 
% zz= roots(q)
% 
...
% subplot(2,1,1)
% 
% semilogx(freq_range,mag2db(abs(H_Y2_full)/abs(H_Y2_full(1))),'LineWidth',2)
% hold on
...
%     ylabel('Mag [dB]')
%     grid on
%     title('End effector Displacement to Shoulder Desired FRF')
%     legend('Experiment','Plot Fitting','-3db')
% 
% subplot(2,1,2)
% if unwrap(angle(siso(1)))/pi*180 > 0
%     coef = -1;
% else
%     coef = 1;
% end
% semilogx(freq_range,unwrap(angle(H_Y2_full))/pi*180,'LineWidth',2)
% hold on
% semilogx(freq_range,unwrap(angle(coef*siso))/pi*180,'LineWidth',2)
% 
% hold on
%     ylim([-200,0])
%     hold off
...
%semilogx(f,mag2db(abs(     (i(1).^2+8*i(1)+x(2)) ./(i.^2+8*i+x(2))       )    ),'LineWidth',2)
semilogx(f,mag2db(abs(     (i(1).^2+8*i(1)+714.28) ./(i.^2+8*i+714.28)       )    ),'LineWidth',2)
...
hold on
    ...
    ylabel('Mag [dB]')
    grid on
    title('End effector Displacement to Shoulder Desired FRF in Shoulder Excitation')
    legend('Experiment','Plot Fitting','-3db')
end
end
